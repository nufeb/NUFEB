...
FixDiffNuGrowth::FixDiffNuGrowth(LAMMPS *lmp, int narg, char **arg) :
		Fix(lmp, narg, arg) {
	if (narg != 53)
		error->all(FLERR, "Not enough arguments in fix diff growth command");

	nevery = force->inumeric(FLERR, arg[3]);
	diffevery = force->inumeric(FLERR, arg[4]);
	outputevery = force->inumeric(FLERR, arg[52]);
	if (nevery < 0 || diffevery < 0 || outputevery < 0)
		error->all(FLERR, "Illegal fix growth command");
	...
	//BC concentration and shear rate
	for (int i = 37; i < 43; i++) {
	...
	if (strcmp(arg[45], "dirich") == 0)
		bflag = 1;
	else if (strcmp(arg[45], "neu") == 0)
		bflag = 2;
	else if (strcmp(arg[45], "mixed") == 0)
		bflag = 3;
	else
		error->all(FLERR, "Illegal boundary condition command");
...
void FixDiffNuGrowth::init() {
	...
	shearRate = input->variable->compute_equal(ivar[42]);
	...
	for (i = xlo - (xstep / 2); i < xhi + xstep; i += xstep) {
		for (j = ylo - (ystep / 2); j < yhi + ystep; j += ystep) {
			for (k = zlo - (zstep / 2); k < zhi + zstep; k += zstep) {
				...
				if (i < xlo || i > xhi || j < ylo || j > yhi || k < zlo
						|| k > zhi) {
					ghost[cell] = true;
...
void FixDiffNuGrowth::change_dia() {
	...
	for (i = 0; i < nall; i++) {
		if (mask[i] & groupbit) {
			...
			int pos = (xpos * (ny + 2) * (nz + 2)) + (ypos * (nz + 2)) + zpos;
	...
	for (int cell = 0; cell < numCells; cell++) {
		//Growth
		R1[cell] = MumHET * (subCell[cell] / (KsHET + subCell[cell]))
				* (o2Cell[cell] / (Ko2HET + o2Cell[cell]));
		R2[cell] = MumAOB * (nh4Cell[cell] / (Knh4AOB + nh4Cell[cell]))
				* (o2Cell[cell] / (Ko2AOB + o2Cell[cell]));
		R3[cell] = MumNOB * (no2Cell[cell] / (Kno2NOB + no2Cell[cell]))
				* (o2Cell[cell] / (Ko2NOB + o2Cell[cell]));
		R4[cell] = etaHET * MumHET * (subCell[cell] / (KsHET + subCell[cell]))
				* (no3Cell[cell] / (Kno3HET + no3Cell[cell]))
				* (Ko2HET / (Ko2HET + o2Cell[cell]));
		R5[cell] = etaHET * MumHET * (subCell[cell] / (KsHET + subCell[cell]))
				* (no2Cell[cell] / (Kno2HET + no2Cell[cell]))
				* (Ko2HET / (Ko2HET + o2Cell[cell]));
		...
		R13[cell] = (1 / 2.86) * bmHET * etaHET
				* (no3Cell[cell] / (Kno3HET + no3Cell[cell]))
				* (Ko2HET / (Ko2HET + o2Cell[cell]));
		R14[cell] = (1 / 1.71) * bmHET * etaHET
				* (no2Cell[cell] / (Kno2HET + no2Cell[cell]))
				* (Ko2HET / (Ko2HET + o2Cell[cell]));

		if (diffevery != 0 && !(update->ntimestep % diffevery)) {
//			Rs[cell] = ((-1 / YHET)
//					* ((R1[cell] + R4[cell] + R5[cell]) * xHET[cell]))
//					+ R6[cell] * xHET[cell] + R7[cell] * xAOB[cell]
//					+ R8[cell] * xNOB[cell] + R9[cell] * xEPS[cell];
			Rs[cell] = ((-1 / YHET)
					* ((R1[cell] + R4[cell] + R5[cell]) * xHET[cell]))
					+ R9[cell] * xEPS[cell];
			Ro2[cell] = (-((1 - YHET - YEPS) / YHET) * R1[cell] * xHET[cell])
					- (((3.42 - YAOB) / YAOB) * R2[cell] * xAOB[cell])
					- (((1.15 - YNOB) / YNOB) * R3[cell] * xNOB[cell]);
			Rnh4[cell] = -(1 / YAOB) * R2[cell] * xAOB[cell];
			Rno2[cell] = ((1 / YAOB) * R2[cell] * xAOB[cell])
					- ((1 / YNOB) * R3[cell] * xNOB[cell])
					- (((1 - YHET - YEPS) / (1.17 * YHET)) * R5[cell]
							* xHET[cell]);
			Rno3[cell] = ((1 / YNOB) * R3[cell] * xNOB[cell])
					- (((1 - YHET - YEPS) / (2.86 * YHET)) * R4[cell]
							* xHET[cell]);
			//Decay and maintenance
			Rs[cell] += (bX * xDEAD[cell]);
			Ro2[cell] = Ro2[cell]
					- ((R10[cell] * xHET[cell]) + (R11[cell] * xAOB[cell])
							+ (R12[cell] * xNOB[cell]));
			Rno2[cell] = Rno2[cell] - (R14[cell] * xHET[cell]);
			Rno3[cell] = Rno3[cell] - (R13[cell] * xHET[cell]);

			double diffusionFunction = 1
					- ((0.43 * pow(xTot[cell], 0.92))
							/ (11.19 + 0.27 * pow(xTot[cell], 0.99)));
	...
	if (!(update->ntimestep % diffevery)) {
		...
		while (!convergence) {
			...
			for (int cell = 0; cell < numCells; cell++) {
				R1[cell] = MumHET * (subPrev[cell] / (KsHET + subPrev[cell]))
						* (o2Prev[cell] / (Ko2HET + o2Prev[cell]));
				R2[cell] = MumAOB * (nh4Prev[cell] / (Knh4AOB + nh4Prev[cell]))
						* (o2Prev[cell] / (Ko2AOB + o2Prev[cell]));
				R3[cell] = MumNOB * (no2Prev[cell] / (Kno2NOB + no2Prev[cell]))
						* (o2Prev[cell] / (Ko2NOB + o2Prev[cell]));
				R4[cell] = etaHET * MumHET
						* (subPrev[cell] / (KsHET + subPrev[cell]))
						* (no3Prev[cell] / (Kno3HET + no3Prev[cell]))
						* (Ko2HET / (Ko2HET + o2Prev[cell]));
				R5[cell] = etaHET * MumHET
						* (subPrev[cell] / (KsHET + subPrev[cell]))
						* (no2Prev[cell] / (Kno2HET + no2Prev[cell]))
						* (Ko2HET / (Ko2HET + o2Prev[cell]));
				//Decay and maintenance
				...
				Rs[cell] = ((-1 / YHET)
						* ((R1[cell] + R4[cell] + R5[cell]) * xHET[cell]))
						+ R9[cell] * xEPS[cell];
				Ro2[cell] =
						(-((1 - YHET - YEPS) / YHET) * R1[cell] * xHET[cell])
								- (((3.42 - YAOB) / YAOB) * R2[cell]
										* xAOB[cell])
								- (((1.15 - YNOB) / YNOB) * R3[cell]
										* xNOB[cell]);
				Rnh4[cell] = -(1 / YAOB) * R2[cell] * xAOB[cell];
				Rno2[cell] = ((1 / YAOB) * R2[cell] * xAOB[cell])
						- ((1 / YNOB) * R3[cell] * xNOB[cell])
				...
				if (!subConvergence)
					compute_flux(cellDs, subCell, subPrev, subBC, Rs[cell],
							diffT, cell);
				if (!o2Convergence)
					compute_flux(cellDo2, o2Cell, o2Prev, o2BC, Ro2[cell],
							diffT, cell);
				if (!nh4Convergence)
					compute_flux(cellDnh4, nh4Cell, nh4Prev, nh4BC, Rnh4[cell],
							diffT, cell);
				if (!no2Convergence)
					compute_flux(cellDno2, no2Cell, no2Prev, no2BC, Rno2[cell],
							diffT, cell);
				if (!no3Convergence)
					compute_flux(cellDno3, no3Cell, no3Prev, no3BC, Rno3[cell],
							diffT, cell);
			}

			if (is_convergence(subCell, subPrev, subBC, tol))
				subConvergence = true;
			if (is_convergence(o2Cell, o2Prev, o2BC, tol))
				o2Convergence = true;
			if (is_convergence(nh4Cell, nh4Prev, nh4BC, tol))
				nh4Convergence = true;
			if (is_convergence(no2Cell, no2Prev, no2BC, tol))
				no2Convergence = true;
			if (is_convergence(no3Cell, no3Prev, no3BC, tol))
				no3Convergence = true;

			if ((subConvergence && o2Convergence && nh4Convergence
					&& no2Convergence && no3Convergence)
					|| iteration == 10000) {
				convergence = true;
			}
		}
	...
	for (int j = 0; j < numCells; j++) {
		caseOneOne[j] = update->dt
				* (R1[j] + R4[j] + R5[j] - R6[j] - R10[j] - R13[j] - R14[j]);
	...
	for (i = 0; i < nall; i++) {
		if (mask[i] & groupbit) {
			...
			density = rmass[i]
					/ (fourThirdsPI * radius[i] * radius[i] * radius[i]);
			rmass[i] = rmass[i] * (1 + (value * nevery));

			if (type[i] == 1) {
				outerMass[i] = fourThirdsPI
						* (outerRadius[i] * outerRadius[i] * outerRadius[i]
								- radius[i] * radius[i] * radius[i]) * EPSdens
						+ value2 * nevery * rmass[i];

				outerRadius[i] = pow(
						threeQuartersPI
								* (rmass[i] / density + outerMass[i] / EPSdens),
						third);
...
bool FixDiffNuGrowth::is_convergence(double *nuCell, double *prevNuCell,
		double nuBC, double tol) {
	for (int cell = 0; cell < numCells; cell++) {
		if (!ghost[cell]) {
...
void FixDiffNuGrowth::compute_flux(double *cellDNu, double *nuCell,
		double *nuPrev, double nuBC, double rateNu, double diffT, int cell) {
	...
	if (ghost[cell]) {
		...
		double jForward = dForward * (nuPrev[forwardCell] - nuPrev[cell])
				/ zstep;
		double dBackward = (cellDNu[cell] + cellDNu[backwardCell]) / 2;
		double jBackward = dBackward * (nuPrev[cell] - nuPrev[backwardCell])
				/ zstep;
		...
		double shear = shearRate * (deep * zstep - zstep / 2)
				* (nuCell[rightCell] - nuCell[leftCell]) / (2 * zstep);

		// Adding fluxes in all the directions and the uptake rate (RHS side of the equation)
		double Ratesub = jX + jY + jZ + rateNu - shear;
...
void FixDiffNuGrowth::compute_Rvalues(double* Rs, double* Ro2, double* Rno2,
		double* Rno3, double* Rnh4) {
	sumRs = 0.0;
	sumRo2 = 0.0;
	sumRno2 = 0.0;
	sumRno3 = 0.0;
...
void FixDiffNuGrowth::output_data(int every, int n) {
	if (!(update->ntimestep % every)) {
		...
		if (n == 6) {
			...
			if (update->ntimestep == every)
				fprintf(pFile,
						"time \t sumRs \t sumRo2 \t sumRno2 \t sumRno3 \t sumRnh4 \n");
			fprintf(pFile, "%i,\t%e,\t%e,\t%e,\t%e,\t%e\n", update->ntimestep,
					sumRs, sumRo2, sumRno2, sumRno3, sumRnh4);
		} else {
				...
				if (!ghost[i]) {
					...
					case 1:
						fprintf(pFile, "%i,\t%f,\t%f,\t%f,\t%f\n", i, xCell[i],
								yCell[i], zCell[i], subCell[i]);
						break;
					case 2:
						fprintf(pFile, "%i,\t%f,\t%f,\t%f,\t%f\n", i, xCell[i],
								yCell[i], zCell[i], o2Cell[i]);
						break;
					case 3:
						fprintf(pFile, "%i,\t%f,\t%f,\t%f,\t%f\n", i, xCell[i],
								yCell[i], zCell[i], no2Cell[i]);
						break;
					case 4:
						fprintf(pFile, "%i,\t%f,\t%f,\t%f,\t%f\n", i, xCell[i],
								yCell[i], zCell[i], no3Cell[i]);
						break;
					case 5:
						fprintf(pFile, "%i,\t%f,\t%f,\t%f,\t%f\n", i, xCell[i],
								yCell[i], zCell[i], nh4Cell[i]);
						break;
					}
				}
			}
