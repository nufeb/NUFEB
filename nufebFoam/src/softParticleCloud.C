...
void softParticleCloud::updateParticles(label lmpNGlobal) {

  Info << "Update particle information..." << endl;

  label foamNGlobal = size();
  reduce(foamNGlobal, sumOp<label>());

  Info << "the number of particles in OpenFOAM now is: " << foamNGlobal << endl;

  if (foamNGlobal != lmpNGlobal) {
...
void softParticleCloud::lammpsEvolveForward(std::vector<vector> &XLocal,
  ...
  Pout << "!!!!!!!!toLmpTagList = " <<toLmpTagListList[0].size() << myrank << endl;
  Pout << "!!!!!!!!toLmpTagList = " <<toLmpTagListList[1].size()<< myrank <<endl;
